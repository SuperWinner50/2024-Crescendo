...
from c import c_code_printers, JavaCodePrinter
...
class CodeGen:
    ...
    def __init__(self, project="project", cse=False, cse_args=None):
    ...
    def routine(self, name, expr, argument_sequence=None, global_vars=None):
        ...
        if self.cse:
            ...
            if is_sequence(expr) and not isinstance(expr, (MatrixBase, MatrixExpr)):
                ...
                common, simplified = cse(rhs, **self.cse_args)

                # pack the simplified expressions back up with their left hand sides
                expr = [Equality(e.lhs, rhs) for e, rhs in zip(expr, simplified)]
            else:
                if isinstance(expr, Equality):
                    common, simplified = cse(expr.rhs, **self.cse_args) #, ignore=in_out_args)
                    expr = Equality(expr.lhs, simplified[0])
                else:
                    common, simplified = cse(expr, **self.cse_args)
                    expr = simplified
...
class JavaCodeGen(CodeGen):
    ...
    def __init__(self, project="project", printer=None,
                 preprocessor_statements=None, cse=False, cse_args=None):
        super().__init__(project=project, cse=cse, cse_args=cse_args)
        self.printer = printer or JavaCodePrinter()
