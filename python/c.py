...
class ListSymbol(MatrixSymbol):
    def _entry(self, i, j, **kwargs):
        return ListElement(self, i, j)

class ListElement(MatrixElement):
    pass

class JavaCodePrinter(C99CodePrinter):
    def _print_MatrixElement(self, expr):
        return "{}.get({}, {})".format(self.parenthesize(expr.parent, PRECEDENCE["Atom"],
            strict=True), expr.i, expr.j)

    def _print_MatrixElementSet(self, lhs, rhs):
        return "{}.set({}, {}, {});".format(self.parenthesize(lhs.parent, PRECEDENCE["Atom"],
            strict=True), lhs.i, lhs.j, self._print(rhs))

    def _print_ListElement(self, expr):
        return "{}[{}]".format(self.parenthesize(expr.parent, PRECEDENCE["Atom"],
            strict=True), expr.j + expr.i*expr.parent.shape[1])

    def _print_sign(self, func):
        return 'signum({})'.format(self._print(func.args[0]))
    ...
    def _print_Assignment(self, expr):
        ...
        if isinstance(expr.rhs, Piecewise):
            ...
            if isinstance(lhs, MatrixElement):
                return self._print_MatrixElementSet(lhs, rhs)
